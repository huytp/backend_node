module Rewards
  class RewardsController < ApplicationController
    ...
    # POST /rewards/update_claimed
    # Update claimed status sau khi node claim thành công trên blockchain
    # Parameters:
    #   - node: node address
    #   - epoch_id: epoch ID
    #   - claimed: boolean
    ...
    def update_claimed
      ...
      if node_address.blank? || epoch_id.blank? || claimed.nil?
        render json: {
          success: false,
          error: 'node, epoch_id, and claimed are required'
        }, status: :bad_request
        return
      end
      ...
      reward.update!(claimed: claimed)

      Rails.logger.info("Updated claimed status for node #{node_address} epoch #{epoch_id}: #{claimed}")
      if params[:tx_hash]
        Rails.logger.info("Transaction: https://amoy.polygonscan.com/tx/#{params[:tx_hash]}")
      end
    ...
    # Lấy danh sách rewards chưa claim cho một node
    def unclaimed
      node_address = params[:node]
      ...
      unclaimed_rewards = Reward.where(node: node, claimed: false)
                                 .joins(:epoch)
                                 .where(epoches: { status: 'committed' })
                                 .order('epoches.epoch_id DESC')

      render json: {
        node: node_address,
        unclaimed_count: unclaimed_rewards.count,
        total_unclaimed_amount: unclaimed_rewards.sum(:amount),
        rewards: unclaimed_rewards.map do |reward|
          {
            epoch_id: reward.epoch.epoch_id,
            amount: reward.amount,
            merkle_root: reward.epoch.merkle_root,
            epoch_end_time: reward.epoch.end_time
          }
        end
      }
    end
